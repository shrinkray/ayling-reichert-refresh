jQuery(document).ready(function($) {
	...
	// colorpicker field
	$('.workz-color-picker').each(function(){
		var $this = $(this),
			id = $this.attr('rel');

		$this.farbtastic('#' + id);
	});
	$('.workz-color-select').click(function(){
		$parent = $(this).parent();
		$(this).siblings('.workz-color-picker').toggle();
		$('.workz-color-select', $parent).toggle();
		return false;
	});

	// add more file
	$('.workz-add-file').click(function(){
	...
	// delete file
	$('.workz-upload').delegate('.workz-delete-file', 'click' , function(){
		var $this = $(this),
			$parent = $this.parent(),
			data = $this.attr('rel');
	...
	$('.workz-images').each(function(){
		...
		$this.sortable({
			...
			update: function (){
				...
				data = order + '|' + $this.siblings('.workz-images-data').val();
	...
	// thickbox upload
	$('.workz-upload-button').click(function(){
	...
	// thickbox upload
    window.formfield = '';
    $('.workz-upload-single-image').live('click', function() {
        window.formfield = $('.workz-single-image',$(this).parent());
